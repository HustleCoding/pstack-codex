...
Launch the configured review-panel count as Codex collaboration agents before waiting. If no config exists, use three when slots allow. Tell each reviewer the task is read-only. Do not pass model slugs unless the collaboration tool explicitly supports them.
