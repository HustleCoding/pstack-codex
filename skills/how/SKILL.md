...
Spawn the explorers as independent Codex collaboration agents before waiting. Tell them the task is read-only. Bound the count by the available collaboration slots and `~/.codex/pstack/config.md`.
...
Explore and explain in the main agent. Read `references/explainer-prompt.md` for the communication style and output format. Use a child agent only when the question is independently large enough to justify delegation.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Once all explorers return, synthesize their findings in the main agent. Read `references/explainer-prompt.md` for the full structure. Reconcile overlap, resolve contradictions against source, and weave the slices into one explanation.
